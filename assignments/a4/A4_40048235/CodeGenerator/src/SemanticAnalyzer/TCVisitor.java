...
public class TCVisitor extends Visitor {
    ...
    private ArrayList<SymbolTableRecord> circularClassRecords;

    public TCVisitor(SymbolTable globalSymbolTable) {
        this.globalSymbolTable = globalSymbolTable;
        circularClassRecords = new ArrayList<>();
    }
...
//                        System.out.println("|\n|  Invalid member call ." + varElement.getSibling().getChildren().getValue() + " in scope" + getScopeMessage(node) + ", variable " + id + " is not a class type");
                        String errorMessage = "tcv, 4.2.6, invalid member call . " + varElement.getSibling().getChildren().getValue() + " in scope" + getScopeMessage(node) + ", variable " + id + " is not a class type";
                        Error.getInstance().insert(node.findLineNumber(), errorMessage, 4);
                        break;
                    }
            ...
            case "multOp": case "addOp": case "assignStat": case "relExpr":
                ...
                if (!leftChildSemanticType.equals(rightChildSemanticType)) {
                    ...
                    String errorMessage = "tcv, 4.2.1, unmatched " + nodeType + " type in scope" + getScopeMessage(node) + ", left is " + leftChildSemanticType + ", right is " + rightChildSemanticType;
                    Error.getInstance().insert(node.findLineNumber(), errorMessage, 4);
                }
                break;
            case "funcDef":
...
//                    System.out.println("|\n|  Missing returnStat in function " + scope + node.getChildren(3).getValue() + ", expect " + declaredReturnType + ", line " + node.findLineNumber());
                    String errorMessage = "tcv, 4.2.1, missing returnStat in function " + scope + node.getChildren(3).getValue() + ", expect " + declaredReturnType;
                    Error.getInstance().insert(node.findLineNumber(), errorMessage, 4);
...
//                    System.out.println("|\n|  Unmatched return type in function " + scope + node.getChildren(3).getValue() + ", expect " + declaredReturnType + ", find " + actualReturnType);
                    String errorMessage = "tcv, 4.2.1, unmatched return type in function " + scope + node.getChildren(3).getValue() + ", expect " + declaredReturnType + ", find " + actualReturnType;
                    Error.getInstance().insert(node.findLineNumber(), errorMessage, 4);
                }
                break;
            case "returnStat":
...
//                        System.out.println("|\n|  Invalid returnStat in main");
                        String errorMessage = "tcv, 4.2.1, invalid returnStat in main";
                        Error.getInstance().insert(node.findLineNumber(), errorMessage, 4);
                        break;
                    }
...
//                        System.out.println("|\n|  Invalid array index type in scope" + getScopeMessage(node) + ", find " + indexNode.getSemanticType());
                        String errorMessage = "tcv, 4.2.4, invalid array index type in scope" + getScopeMessage(node) + ", find " + indexNode.getSemanticType();
                        Error.getInstance().insert(node.findLineNumber(), errorMessage, 4);
                    }
            ...
            case "classList":
                // 4.2.5
                // remove all circular class record
                for (SymbolTableRecord circularClassRecord : circularClassRecords) {
                    globalSymbolTable.removeRecord(circularClassRecord);
                }
                break;
        }
    }
...
//                System.out.println("|\n|  Undefined free function " + id + " in scope" + getScopeMessage(node));
                String errorMessage = "tcv, 4.2.8, undeclared free function " + id + " in scope" + getScopeMessage(node);
                Error.getInstance().insert(node.findLineNumber(), errorMessage, 4);
            } else {
//                System.out.println("|\n|  Undefined local variable " + id + " in scope" + getScopeMessage(node));
                String errorMessage = "tcv, 4.2.10, undefined local variable " + id + " in scope" + getScopeMessage(node);
                Error.getInstance().insert(node.findLineNumber(), errorMessage, 4);
            }
            return "Unknown";
        } else {
...
//                        System.out.println("|\n|  Undefined member function " + funcName + " in scope" + getScopeMessage(node));
                        String errorMessage = "tcv, ???, undeclared member function " + id + " in scope" + getScopeMessage(node);
                        Error.getInstance().insert(node.findLineNumber(), errorMessage, 4);
                        return "Unknown";
                    }
                }
//                System.out.println("|\n|  Undefined member variable " + id + " in scope" + getScopeMessage(node));
                String errorMessage = "tcv, ???, undeclared data member " + id + " in scope" + getScopeMessage(node);
                Error.getInstance().insert(node.findLineNumber(), errorMessage, 4);
            } else {
                for (SymbolTableRecord globalRecord : globalSymbolTable.getSymbolTableRecords()) {
...
//                        System.out.println("|\n|  Undefined member function " + id + " in scope" + getScopeMessage(node));
                        String errorMessage = "tcv, 4.2.8, undeclared member function " + id + " in scope" + getScopeMessage(node);
                        Error.getInstance().insert(node.findLineNumber(), errorMessage, 4);
                        return "Unknown";
                    }
...
//                System.out.println("|\n|  Undefined member variable " + id + " in scope" + getScopeMessage(node));
                String errorMessage = "tcv, 4.2.11, undeclared data member " + id + " in scope" + getScopeMessage(node);
                Error.getInstance().insert(node.findLineNumber(), errorMessage, 4);
            }
...
//                    }
                    if (!recordType.contains("float") && !recordType.contains("integer") && dependencyTableRecord.getKind().equals("variable")) {
                        dependenciesList.add(recordType);
                    }
                }
                break;
                ...
                if (globalRecord.getName().equals(dependency) && globalRecord.getKind().equals("class")) {
                    ...
                    if (!dependencyInherList.isEmpty()) {
                        ...
                        if (inher.equals(className)) {
                            System.out.println("|\n|  4.2.5 Circular class dependencies in class " + className);
                            // TODO: remove class record from global table
                            circularClassRecords.add(globalSymbolTable.findRecord(className, "class"));
                        }
                    }
    ...
    private void checkFuncParamList(String scopeClassName, String funcName, ASTNode fCall) {
        ...
        if (null != funcRecord) {
            ...
            if (!funcDefParamType.equals(funcCallParamType.toString())) {
                ...
                System.out.println("|\n|  4.2.2 Unmatched function call parameter in scope" + getScopeMessage(fCall) + ", expect " + scopedFuncName + "(" + funcDefParamType + "), find " + scopedFuncName + "(" + funcCallParamType + ")");
            }
        }
    }
...
//            System.out.println("|\n|  Unmatched variable dimension in scope" + getScopeMessage(node) + ", expect " + varDeclDimSize + ", find " + varDimSize);
            String errorMessage = "tcv, 4.2.3, unmatched variable dimension in scope" + getScopeMessage(node) + ", expect " + varDeclDimSize + ", find " + varDimSize;
            Error.getInstance().insert(node.findLineNumber(), errorMessage, 4);
        }
    }
}
