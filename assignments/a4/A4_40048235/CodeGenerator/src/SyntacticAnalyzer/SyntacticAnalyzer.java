            ...
            if (terminalSet.isTerminal(x)) {
                ...
                if (x.equals(a)) {
                    ...
                    if (!error) {
                        ...
                        Error.getInstance().insert(lineNumber, syntacticError, 3);
...
//                    }
        ...
        lexicalAnalyzer.generateTokenStreamFile();
        generateOutputFiles();
        return ast;
    }
    ...
    private void generateOutputFiles() {
        ...
        BufferedWriter astBW = null;
        try {
            File derivationsFile = new File("test/3_derivations/" + filename);
            File astFile = new File("test/3_AST/" + filename);
            derivationsBW = new BufferedWriter(new FileWriter(derivationsFile));
            astBW = new BufferedWriter((new FileWriter(astFile)));
            derivationsBW.write(derivations.toString());
            String astContent = ast.print();
            astBW.write(astContent);
        } catch (IOException e ) {
            System.out.println("ERROR 1 at SyntacticAnalyzer.SyntacticAnalyzer.writeDerivation(): " + e.getMessage());
        } finally {
            if (null != derivationsBW && null != astBW) {
                try {
                    derivationsBW.close();
                    astBW.close();
                } catch (IOException e) {
