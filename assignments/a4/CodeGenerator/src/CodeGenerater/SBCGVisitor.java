...
public class SBCGVisitor { // StackBasedCodeGenerationVisitor
    ...
    public void visit(ASTNode node) {
        ...
        switch (node.getType()) {
            ...
            case "forStat": {
                String localRegister1 = registers.pop();
                String localRegister2 = registers.pop();


                assemblyCode += cmtIndent + "%processing forStat\n";

                // expr node
//                node.getChildren(3).accept(this);
                String temp = node.getChildren(3).getChildren().getChildren().getChildren().getChildren().getValue();
                assemblyCode += codeIndent + "addi " + localRegister1 + ", r0, " + temp + "\n";
                assemblyCode += codeIndent + "sw -9996(r14), " + localRegister1 + "\n";

                // relExprNode
//                node.getChildren(4).accept(this);
//                assemblyCode += codeIndent + "lw " + localRegister1 + ", -9996(r14)\n";

                // assignStat
//                node.getChildren(5).accept(this);

                assemblyCode += "beginFor\n";
                assemblyCode += codeIndent + "lw "   + localRegister1 + ", -9996(r14)\n";
                assemblyCode += codeIndent + "bz " + localRegister1 + ", endFor\n";

                assemblyCode += codeIndent + "subi " + localRegister2 + ", " + localRegister1 + ", 1\n";
                assemblyCode += codeIndent + "sw -9996(r14), " + localRegister2 + "\n";

                // statBlock
                node.getChildren(6).accept(this);
                assemblyCode += codeIndent + "j beginFor\n";
                assemblyCode += "endFor\n";

                registers.push(localRegister2);
                registers.push(localRegister1);
                return;
            }
        }
        ...
        switch (nodeType) {
            ...
            case "fCall": {
                ...
                while (null != funcParam) {
                    ...
                    // TODO: suppose it's free function call
                    if (null != parent && parent.getType().equals("funcDef")) {
                        funcParamOffset += globalTable.findRecord(parent.getChildren(3).getValue(), "function").getOffset();
                    } else if (null != parent && parent.getType().equals("prog")) {
                            ...
                            while (null != parent) {
                                switch (parent.getType()) {
                                    ...
                                    case "forStat":
                                        ...
                                        parent.getChildren(4).getChildren().setTagAndPassToParent(-9996);
                                        //tagIndex -= 4;
