...
class AST {
    ...
    AST(Stack<ASTNode> semanticStack) {
        this.semanticStack = semanticStack;
    }

    private void makeNode(String type, String value) { semanticStack.push(new ASTNode(type, value)); }
    ...
    private void makeFamily(String parentName, int childrenSize) {
        ...
        if (-1 == childrenSize) {
            ...
            for (int i = 0; i < childrenSize-1; ++i) {
                if (1 == i && (parentName.equals("addOp") || parentName.equals("multOp"))) {
                    parent.setValue(semanticStack.pop().getValue());
                } else {
                    makeSiblings();
                }
            }
